    ...
    <div className="max-w-md mx-auto">
      ...
      <form onSubmit={handleSubmit} className="bg-white rounded-xl border border-gray-200 p-8 space-y-4">
        ...
        <button type="submit" disabled={loading}
          className="w-full py-2.5 btn-brand rounded-lg text-sm font-medium disabled:opacity-50">
          {loading ? 'Logging in…' : 'Login'}
        </button>
        <p className="text-center text-sm text-gray-500">
          First time? <a href="/register" className="text-brand hover:underline">Activate web access</a>
        </p>
      </form>
    </div>
  );
