...
describe('Staff nav group', () => {
  it('has a Staff group with the 3 expected items', () => {
    const group = NAV_GROUPS.find((g) => g.title === 'Staff');
    expect(group).toBeDefined();
    const hrefs = group!.items.map((i) => i.href);
    expect(hrefs).toEqual(['/settings/staff', '/settings/permissions', '/staff/live-monitor']);
  });
  ...
  it('does not include an Attendance item anywhere in NAV_GROUPS (Phase 2 not started)', () => {
    const allItems = NAV_GROUPS.flatMap((g) => g.items);
    const attendance = allItems.find(
      (i) => i.label.toLowerCase().includes('attendance') || i.href.includes('/staff/attendance'),
    );
  ...
  it('SuperAdmin sees every Staff item regardless of granted permissions', () => {
    ...
    expect(staffGroup!.items).toHaveLength(3);
  });
