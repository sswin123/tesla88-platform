...
export async function POST(
  ...
  try {
    ...
    const { rows } = await client.query<{
      ...
      `SELECT wr.id, wr.status, wr.processing_by, a.erp_username AS processing_by_name
    ...
    if (row.status !== 'PENDING') {
      await client.query('ROLLBACK');
      return NextResponse.json({ error: 'Withdrawal is no longer pending', status: row.status }, { status: 409 });
    }
    ...
    console.error('[withdrawals/process]', err);
