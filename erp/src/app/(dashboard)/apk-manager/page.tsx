      ...
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-8">
        <div>
          ...
          <form onSubmit={submit} className="bg-card rounded-xl border border-border p-6 space-y-4">
            ...
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">Release Notes</label>
              <textarea value={form.release_notes} rows={3}
                onChange={e => setForm(prev => ({ ...prev, release_notes: e.target.value }))}
                className="w-full border border-border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
            </div>
            <div>
              <label className="block text-sm font-medium text-foreground mb-1">APK File</label>
        ...
        <div>
          ...
          {versions.length === 0 ? (
            <div className="bg-card rounded-xl border border-border p-8 text-center text-muted-foreground text-sm">No APK versions yet.</div>
          ) : (
            <div className="space-y-3">
              {versions.map(v => (
                <div key={v.id} className={`bg-card rounded-xl border p-4 ${v.is_current ? 'border-blue-400 ring-1 ring-blue-200' : 'border-border'}`}>
                  <div className="flex items-start justify-between gap-2">
                    <div>
                      ...
                      <p className="text-xs text-muted-foreground mt-0.5">Build {v.version_code} · Android {v.min_android}+ · {v.download_count} downloads</p>
                      {v.release_notes && <p className="text-xs text-muted-foreground mt-1 line-clamp-2">{v.release_notes}</p>}
                    </div>
