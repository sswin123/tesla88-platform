...
export class ActivityLogService {
  /**
   * Log a member activity event. Never throws — failures are logged and swallowed
   * so they never block the caller's business operation.
   *
   * Returns the generated activity_id, or null if the write failed.
   */
  static async log(input: LogActivityInput): Promise<string | null> {
    ...
    try {
      ...
      // Activity log failures must NEVER block business operations
